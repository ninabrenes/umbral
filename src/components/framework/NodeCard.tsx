...
export function NodeCard({ node, locale, index }: NodeCardProps) {
  return (
    <Link
      ...
      className="group block py-6 border-b border-ink/[0.06] last:border-b-0"
    >
      <div className="flex items-start gap-5">
        <div className="shrink-0 mt-1 text-moss/70 group-hover:text-sage transition-colors">
          <NodeIcon nodeId={node.id} size={28} />
        </div>
