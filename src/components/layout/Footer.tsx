...
export function Footer({ locale }: FooterProps) {
  ...
  return (
    <footer className="bg-onyx pb-16">
      <div className="mx-auto max-w-[1400px] px-6 md:px-12 lg:px-20 pt-24 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8">
          {/* Brand */}
          <div className="md:col-span-5">
            <p className="font-serif text-2xl font-light text-cloud mb-4">
              umbral
            </p>
            <p className="text-sm font-light leading-relaxed max-w-[40ch] text-cloud/70">
              {locale === 'en'
          ...
          <div className="md:col-span-2">
            <p className="text-xs tracking-[0.15em] uppercase text-mint/60 mb-5">
              ...
              {footerLinks.framework.map((link) => (
                <li key={link.key}>
                  <Link
                    ...
                    className="text-sm font-light text-sage hover:text-mint transition-colors"
                  >
                    {link[locale]}
                  </Link>
                </li>
        ...
        <div className="mt-20 pt-8 border-t border-white/[0.08]">
          <p className="text-xs text-cloud/40 font-light leading-relaxed max-w-[80ch]">
            {disclaimer}
          </p>
          <p className="mt-4 text-xs text-cloud/40 font-light">
            {copyright}
          </p>
        </div>
      </div>
